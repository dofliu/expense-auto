...
def navigate_to_expense_form(menu_page: Page, use_project: bool = False,
    ...
    # Step 1.5 (計畫請購): 從 TITLE BUGETNO 下拉選擇計畫
    if use_project and plan_name:
        title_frame = menu_page.frame("TITLE")
        title_frame.wait_for_load_state("networkidle", timeout=10000)
        time.sleep(1)

        plan_esc = _js_escape(plan_name)
        result = title_frame.evaluate(f"""() => {{
            const sel = document.querySelector('select[name="BUGETNO"]');
            if (!sel) return {{ found: false, reason: 'BUGETNO dropdown not found' }};

            const opts = [...sel.options].map((o, i) => ({{
                i: i, v: o.value, t: o.text
            }}));

            for (let i = 0; i < sel.options.length; i++) {{
                const t = sel.options[i].text;
                const v = sel.options[i].value;
                if (t.includes('{plan_esc}') || v.includes('{plan_esc}')) {{
                    sel.selectedIndex = i;
                    sel.dispatchEvent(new Event('change'));
                    return {{ found: true, index: i, value: v, text: t }};
                }}
            }}
            return {{ found: false, reason: 'keyword not found: {plan_esc}',
                      options: opts.slice(0, 20) }};
        }}""")

        if result.get("found"):
            print(f"    TITLE 計畫: {result.get('text', '')}")
        else:
            print(f"    警告: TITLE BUGETNO 找不到「{plan_name}」")
            if result.get("options"):
                for opt in result["options"][:10]:
                    print(f"      - [{opt['v']}] {opt['t']}")
        time.sleep(1)
...
def generate_receipt_no(iso_date: str, seq: int) -> str:
    ...
    """
    y, m, d = _roc_date(iso_date)
    return f"{RECEIPT_PREFIX}{y}{m}{d}{seq:02d}"
...
def fill_appy_frame(appy_frame: Frame, menu_page: Page,
    ...
    options_data = appy_frame.evaluate("""() => {
        const sel = FORM1.BUGETNO_1;
        if (!sel || sel.options.length <= 1) return [];
        return [...sel.options].map((o, idx) => ({ index: idx, text: o.text, value: o.value }));
    }""")
    
    # 過濾出有值的選項（排除 "請下拉選擇計畫" 或空值）
    valid_options = [o for o in options_data if o['value'] and o['value'].strip() and o['index'] > 0]
    ...
    if valid_options:
        ...
        if selected_idx == -1:
            ...
            print(f"    → 最終選擇計畫: {options_data[selected_idx]['text']}")
...
def fill_appa_frame(appa_frame: Frame, menu_page: Page, context,
    ...
    # ── Step 2: 填入收據/發票號碼 ────────────────
    # 如果 OCR 辨識到發票號碼（如 XC-12345678）就用發票號碼
    # 否則自動產生收據號碼（收據115022601）
    ...
    appa_frame.evaluate(f"""() => {{
        const el = FORM1.VENDORID_S_1;
        if (el) {{
            el.value = '{payee_esc}';
            el.dispatchEvent(new Event('change', {{bubbles:true}}));
            el.dispatchEvent(new Event('blur', {{bubbles:true}}));
        }}
        // STAR_ID_1() 將 VENDORID_S_1 同步到 VENDORID_1
    ...
    try:
        ...
        if result.get("found"):
            ...
            appa_frame.evaluate(f"""() => {{
                const fire = (el) => {{ if(el) {{ el.dispatchEvent(new Event('change', {{bubbles:true}})); el.dispatchEvent(new Event('blur', {{bubbles:true}})); }} }};
                FORM1.VENDORID_S_1.value = '{vs}'; fire(FORM1.VENDORID_S_1);
                FORM1.VENDORID_1.value = CharToAsc('{vs}'); fire(FORM1.VENDORID_1);
                FORM1.VENNAME_1.value = '{vn}'; fire(FORM1.VENNAME_1);
                FORM1.BANKNO_1.value = '{bk}'; fire(FORM1.BANKNO_1);
                FORM1.ACCOUNT_1.value = '{ac}'; fire(FORM1.ACCOUNT_1);
                FORM1.ACCOUNTNAM_1.value = '{an}'; fire(FORM1.ACCOUNTNAM_1);
            }}""")
...
def verify_and_save(appy_frame: Frame, menu_page: Page):
    """
    驗證三個區塊的金額一致後，自動點擊存入。
    ...
    for label, js_code in button_cycle:
    ...
    # ── Step 5: 攔截 PS frame 的 submit ──
    # 用來診斷 D_STR/P_STR/A_STR 的內容
    appy_frame.evaluate("""() => {
        try {
            window._submit_hooked = false;
            const originalSubmit = parent.PS.FORM1.submit;
            parent.PS.FORM1.submit = function() {
                window._last_submit_data = {
                    ...
                    D_len: parent.PS.FORM1.D_STR ? parent.PS.FORM1.D_STR.value.length : 0,
                    P_len: parent.PS.FORM1.P_STR ? parent.PS.FORM1.P_STR.value.length : 0,
                    A_len: parent.PS.FORM1.A_STR ? parent.PS.FORM1.A_STR.value.length : 0,
                };
                // 真正存入
                originalSubmit.call(parent.PS.FORM1);
            };
            window._submit_hooked = true;
        } catch(e) {}
    }""")

    # ── Step 6: 呼叫 SUM_ALERT() 存入 ──
    try:
    ...
    # 等待足夠長的時間（最長 15 秒）以處理：
    #   - SUM_ALERT 的序列化和 submit
    #   - 伺服器處理
    #   - 任何後續 dialog（PDF、確認、錯誤等）
    print("    等待存入完成...")
    for sec in range(15):
        time.sleep(1)
        if (sec + 1) % 5 == 0:
    ...
    # 取得攔截到的資料
    try:
        submit_debug = appy_frame.evaluate("() => window._last_submit_data || {}")
    except Exception:
        submit_debug = {}

    if submit_debug and submit_debug.get("D_len", 0) > 0:
        print(f"    [DEBUG] D_STR ({submit_debug.get('D_len', 0)} chars): "
              f"{str(submit_debug.get('D_STR', ''))[:80]}...")
        print(f"    [DEBUG] P_STR ({submit_debug.get('P_len', 0)} chars): "
              f"{str(submit_debug.get('P_STR', ''))[:80]}...")
        print(f"    [DEBUG] A_STR ({submit_debug.get('A_len', 0)} chars): "
              f"{str(submit_debug.get('A_STR', ''))[:80]}...")

        # 檢查 P_STR 和 A_STR 是否有內容
        p_len = submit_debug.get("P_len", 0)
        a_len = submit_debug.get("A_len", 0)
        if p_len == 0:
            print("    [WARN] P_STR 為空！APPP 品名資料未被序列化。")
        if a_len == 0:
            print("    [WARN] A_STR 為空！APPA 受款人資料未被序列化。")
    else:
        print("    [DEBUG] 未攔截到 submit（可能 SUM_ALERT 提早 return）")

    # 檢查 PS frame 是否有回應
    try:
        ps_body = ""
        for f in menu_page.frames:
            if f.name == "PS" or "SAVE" in f.url.upper():
                ps_body = f.evaluate(
                    "document.body ? document.body.innerText.substring(0, 300) : ''"
                )
                if ps_body:
                    print(f"    [PS回應] {ps_body[:200]}")
                break
    except Exception:
        pass

    # 移除 dialog handler 避免影響後續
    try:
...
def verify_saved_record(menu_page: Page, expected_amount: int = 0):
    ...
    title_frame = menu_page.frame("TITLE")
    if not title_frame:
        print("    [FAIL] 找不到 TITLE frame")
        return {"ok": False, "error": "no TITLE frame"}
    ...
    if not main_frame:
        print("    [FAIL] 找不到 MAIN frame")
        return {"ok": False, "error": "no MAIN frame"}
    ...
    if result["ok"]:
        print(f"  [OK] 記錄 {result['record_no']} 驗證通過！品名和受款人都有正確存入。")
    else:
        print(f"  [FAIL] 記錄 {result['record_no']} 驗證失敗！部分資料遺失。")
...
def fill_expense_form(frames: dict, receipt_data: dict,
    ...
    for i, item_data in enumerate(parsed_items, start=1):
        item_name = _js_escape(item_data["name"])
        item_spec = _js_escape(item_data["spec"])
    ...
    # ── 6. 驗證三金額一致 → 自動存入 ────────────────
    if auto_save and appy_frame and menu_page:
        saved = verify_and_save(appy_frame, menu_page)
        if saved:
            print("  [OK] 表單已自動存入")
            # ── 7. 回到購案管理驗證存入內容 ────────────
            verify_result = verify_saved_record(menu_page, expected_amount=total_amount)
            if verify_result.get("ok"):
                print(f"  [OK] 驗證通過！記錄 {verify_result.get('record_no', '')}")
            else:
                print(f"  [WARN] 驗證未通過，請手動確認")
        else:
            print("  [FAIL] 自動存入失敗，請手動確認")
    else:
        print("  表單填寫完成（未啟用自動存入）")
